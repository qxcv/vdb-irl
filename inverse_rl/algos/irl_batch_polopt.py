...
from inverse_rl.utils.log_utils import prune_old_snapshots
...
class IRLBatchPolopt(RLAlgorithm, metaclass=Hyperparametrized):
    ...
    def __init__(self,
        ...
        :param scope: Scope for identifying the algorithm. Must be specified if
            running multiple algorithms simultaneously, each using different
        ...
        self.snap_keep_every = snap_keep_every
        self.snap_keep_latest = snap_keep_latest
    ...
    def compute_irl(self, paths, itr=0):
        if self.irl_model is not None or self.no_reward:
            tot_rew = 0
            for path in paths:
        ...
        if self.train_irl:
            ...
            mean_loss = self.irl_model.fit(
                ...
                last_timestep_only=self.last_state_only)
        ...
        logger.record_tabular('IRLRewardMean', np.mean(np.concatenate(probs)))
        logger.record_tabular('IRLRewardMax', np.max(np.concatenate(probs)))
        logger.record_tabular('IRLRewardMin', np.min(np.concatenate(probs)))

        if self.last_state_only:
            for i, path in enumerate(paths):
                # path['rewards'][-1] += self.irl_model_wt * probs[i][-1]
                path['rewards'] += self.irl_model_wt * probs[i]
        else:
            if self.irl_model.score_trajectories:
                # TODO: should I add to reward here or after advantage
                # computation?
                for i, path in enumerate(paths):
