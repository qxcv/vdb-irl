...
from inverse_rl.models.architectures import feedforward_energy
from multiple_irl.models.architectures import relu_net
from inverse_rl.models.tf_util import discounted_reduce_sum
from inverse_rl.utils.general import TrainingIterator, kl_loss
...
class ImitationLearning(object, metaclass=Hyperparametrized):
    ...
    def set_demos(self, paths, last_timestep_only=False):
        if paths is not None:
            self.expert_trajs = paths
            self.expert_trajs_extracted = self.extract_paths(
                paths, last_timestep_only=last_timestep_only)
    ...
    def _insert_next_state(paths, pad_val=0.0):
        for path in paths:
            ...
            nact = path['actions'][1:]
            nobs = np.r_[nobs, pad_val * np.expand_dims(
                np.ones_like(nobs[0]), axis=0)]
            nact = np.r_[nact, pad_val * np.expand_dims(
                np.ones_like(nact[0]), axis=0)]
    ...
    def extract_paths(paths,
        ...
        if last_timestep_only:
            assert not stack, "Trajectory mode not allowed for last_timestep_only"
            result = [
                np.array([t[key][-1] for t in paths]).astype(np.float32)
                for key in keys
            ]
            return result
        if stack:
    ...
    def sample_batch(*args, batch_size=32):
        ...
        batch_idxs = np.random.randint(
            0, N, batch_size)  # trajectories are negatives
    ...
    def _make_param_ops(self, vs):
        ...
        self._assign_plc = [
            tf.placeholder(
                ...
                name='assign_%s' % param.name.replace('/', '_').replace(':',
                                                                        '_'))
            for param in self._params
        ]
        self._assign_ops = [
            tf.assign(self._params[i], self._assign_plc[i])
...
class SingleTimestepIRL(ImitationLearning):
    ...
    def extract_paths(paths,
                      ...
                      stack=False,
                      last_timestep_only=False):
        return ImitationLearning.extract_paths(
            paths,
            keys=keys,
            stack=stack,
...
class GAIL(SingleTimestepIRL):
    ...
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            if vail:
                if self.frozen:
                    # for frozen models we can use garbage params
                    if vail_init_beta is None:
                        vail_init_beta = 0.0
                    if vail_beta_step_size is None:
                        vail_beta_step_size = 0.0
                    if vail_kl_target is None:
                        vail_kl_target = 0.0
                # otherwise, we need reasonable ones to be given
                assert vail_init_beta is not None
                assert vail_beta_step_size is not None
                assert vail_kl_target is not None
                self.vail_initial_beta = vail_init_beta
                self.vail_beta_step_size = vail_beta_step_size
                self.vail_kl_target = vail_kl_target
                # now update ops
                self.vail_beta = tf.get_variable(
                    'vail_beta',
                    trainable=False,
                    shape=(),
                    dtype=tf.float32,
                    initializer=tf.initializers.constant(vail_init_beta))
                self.vail_mean_kl = tf.placeholder(
                    name='vail_kl_ph', shape=(), dtype=tf.float32)
                beta_new_value = tf.maximum(
                    0.0, self.vail_beta + self.vail_beta_step_size *
                    (self.vail_mean_kl - self.vail_kl_target))
                self.vail_beta_update_op = tf.assign(
                    self.vail_beta, beta_new_value)
            ...
            if vail:
                logits, vail_mean, vail_logstd = discrim_arch(
                    obs_act, **discrim_arch_args)
                vail_kl_loss = kl_loss(vail_mean, vail_logstd)
            else:
            ...
            if vail:
                self.tot_kl_loss = vail_kl_loss
                self.loss += self.vail_beta * self.tot_kl_loss
            else:
    ...
    def fit(self, trajs, batch_size=32, max_itrs=100, **kwargs):
        ...
        for it in TrainingIterator(max_itrs, heartbeat=5):
            ...
            if self.vail:
                beta, _ = sess.run(
                    [self.vail_beta, self.vail_beta_update_op],
                    feed_dict={self.vail_mean_kl: tot_kl})
    ...
    def eval(self, paths, **kwargs):
        ...
        scores = tf.get_default_session().run(
            self.predictions, feed_dict={self.act_t: acts,
                                         self.obs_t: obs})
    ...
    def debug_eval(self, obs, acts, **kwargs):
        scores = tf.get_default_session().run(
            self.predictions, feed_dict={self.act_t: acts,
                                         self.obs_t: obs})
...
class FrozenGAIL(SingleTimestepIRL):
    """
    Generative adverserial imitation learning
    See https://arxiv.org/pdf/1606.03476.pdf
    This version consumes single timesteps.
    ...
    def __init__(self,
                 ...
                 discrim_arch=relu_net,
                 discrim_arch_args={},
                 freeze_after_n_iters=1,
                 max_train_itrs=100,
                 name='gail'):
        super(FrozenGAIL, self).__init__()
        self.dO = env_spec.observation_space.flat_dim
        self.dU = env_spec.action_space.flat_dim
        self.freeze_after_n_iters = freeze_after_n_iters
        self.max_train_iters = max_train_itrs
        self.set_demos(expert_trajs, last_timestep_only=True)
        ...
        with tf.variable_scope(name) as vs:
            ...
            #obs_act = tf.concat([self.obs_t, self.act_t], axis=1)
            logits = discrim_arch(self.obs_t, **discrim_arch_args)
            self.predictions = tf.nn.sigmoid(logits)
            self.loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=logits, labels=self.labels))
    ...
    def fit(self, trajs, batch_size=32, **kwargs):
        self._num_fit += 1
        if self._num_fit > self.freeze_after_n_iters:
            return -1.0

        obs, acts = self.extract_paths(trajs)
        expert_obs, expert_acts = self.expert_trajs_extracted
        num_expert_states = expert_obs.shape[0]
        num_nonexpert_states = obs.shape[0]
        assert (num_nonexpert_states >= num_expert_states)

        rand = np.random.choice(num_nonexpert_states, num_expert_states)
        obs = obs[rand]
        acts = acts[rand]

        # Train discriminator
        for it in TrainingIterator(self.max_train_iters, heartbeat=5):
            obs_batch, act_batch = self.sample_batch(
                obs, acts, batch_size=batch_size)
            expert_obs_batch, expert_act_batch = self.sample_batch(
                expert_obs, expert_acts, batch_size=batch_size)
    ...
    def __init__(self,
        ...
        with tf.variable_scope(name) as _vs:
            ...
            with tf.variable_scope('discrim') as dvs:
                with tf.variable_scope('energy'):
                    energy = discrim_arch(
                        obs_act, dout=self.dU, **discrim_arch_args)

                self.value_fn = tf.reduce_logsumexp(
                    -energy, axis=1, keep_dims=True)
                self.energy = tf.reduce_sum(
                    energy * self.act_t, axis=1,
    ...
    def __init__(self,
        ...
        with tf.variable_scope(name) as vs:
            ...
            if discount >= 1.0:
                ...
                log_p_tau = discounted_reduce_sum(
                    -self.energy, discount=discount, axis=1)
