...
class CustomGymEnv(RllabGymEnv):
    ...
    def log_diagnostics(self, paths):
        inner_env = get_inner_env(self.env)
        if hasattr(inner_env, 'log_diagnostics'):
            inner_env.log_diagnostics(paths)
        else:
            warnings.warn(
                '%s environment has no log_diagnostics() support' % inner_env,
                stacklevel=2)
